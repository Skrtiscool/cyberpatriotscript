# test-group-management.ps1
# Unit tests for group-management.ps1

param(
    [switch]$Verbose
)

$ErrorActionPreference = "Stop"
$scriptPath = Split-Path -Parent $MyInvocation.MyCommand.Path
$refactorPath = Join-Path $scriptPath "..\refactor"

function Test-FileExists {
    param([string]$Path)
    if (Test-Path $Path) {
        Write-Host "✓ File exists: $Path" -ForegroundColor Green
        return $true
    } else {
        Write-Host "✗ File not found: $Path" -ForegroundColor Red
        return $false
    }
}

function Test-ValidActions {
    Write-Host "`nTesting valid actions..." -ForegroundColor Cyan
    
    $scriptContent = Get-Content (Join-Path $refactorPath "group-management.ps1") -Raw
    
    $expectedActions = @(
        'List'
        'ListMembers'
        'AddMember'
        'RemoveMember'
    )
    
    $allFound = $true
    foreach ($action in $expectedActions) {
        if ($scriptContent -like "*`"$action`"*" -or $scriptContent -like "*'$action'*") {
            Write-Host "  ✓ Action defined: $action" -ForegroundColor Green
        } else {
            Write-Host "  ✗ Action missing: $action" -ForegroundColor Red
            $allFound = $false
        }
    }
    
    return $allFound
}

function Test-RequiredFunctions {
    Write-Host "`nTesting function definitions..." -ForegroundColor Cyan
    
    $scriptContent = Get-Content (Join-Path $refactorPath "group-management.ps1") -Raw
    
    $requiredFunctions = @(
        'Get-LocalGroupsList'
        'Get-GroupMembers'
        'Add-UserToGroup'
        'Remove-UserFromGroup'
    )
    
    $allFound = $true
    foreach ($func in $requiredFunctions) {
        if ($scriptContent -like "*function $func*") {
            Write-Host "  ✓ Function defined: $func" -ForegroundColor Green
        } else {
            Write-Host "  ✗ Function missing: $func" -ForegroundColor Red
            $allFound = $false
        }
    }
    
    return $allFound
}

function Test-LocalGroupCommands {
    Write-Host "`nTesting local group commands..." -ForegroundColor Cyan
    
    $scriptContent = Get-Content (Join-Path $refactorPath "group-management.ps1") -Raw
    
    $commands = @(
        'Get-LocalGroup'
        'Get-LocalGroupMember'
        'Add-LocalGroupMember'
        'Remove-LocalGroupMember'
    )
    
    $allFound = $true
    foreach ($cmd in $commands) {
        if ($scriptContent -like "*$cmd*") {
            Write-Host "  ✓ Command found: $cmd" -ForegroundColor Green
        } else {
            Write-Host "  ✗ Command missing: $cmd" -ForegroundColor Red
            $allFound = $false
        }
    }
    
    return $allFound
}

function Test-FallbackCommands {
    Write-Host "`nTesting fallback net commands..." -ForegroundColor Cyan
    
    $scriptContent = Get-Content (Join-Path $refactorPath "group-management.ps1") -Raw
    
    if ($scriptContent -like "*net localgroup*") {
        Write-Host "  ✓ Fallback net localgroup command found" -ForegroundColor Green
        return $true
    } else {
        Write-Host "  ✗ Fallback net localgroup command missing" -ForegroundColor Red
        return $false
    }
}

# Run tests
Write-Host "======================================" -ForegroundColor Cyan
Write-Host "Testing: group-management.ps1" -ForegroundColor Cyan
Write-Host "======================================" -ForegroundColor Cyan

$results = @()
$results += (Test-FileExists (Join-Path $refactorPath "group-management.ps1"))
$results += (Test-ValidActions)
$results += (Test-RequiredFunctions)
$results += (Test-LocalGroupCommands)
$results += (Test-FallbackCommands)

Write-Host "`n======================================" -ForegroundColor Cyan
$passed = ($results | Where-Object { $_ -eq $true }).Count
$total = $results.Count
Write-Host "Results: $passed/$total tests passed" -ForegroundColor $(if ($passed -eq $total) { "Green" } else { "Yellow" })
Write-Host "======================================" -ForegroundColor Cyan
