@echo off
REM mrsimpon.bat - Toolbox snippet with embedded PowerShell to apply recommended password & lockout policy
REM Created to be self-contained: writes a PS1 to %%TEMP%% and runs it elevated
...
cls
echo ================================================
echo   Toolbox: Security / Policy Utilities
echo ================================================
echo 1) Apply recommended password & lockout policy
echo 2) Verify current password & lockout settings (net accounts + secedit export)
echo 0) Exit
echo.
set /p choice=Select an option: 

if "%choice%"=="1" goto apply_policy
if "%choice%"=="2" goto verify_policy
...
:apply_policy
echo Preparing PowerShell policy apply script in %%TEMP%% ...

powershell -NoProfile -ExecutionPolicy Bypass -Command "Set-Content -LiteralPath \"$env:TEMP\"\ + '\\toolbox_apply_policy.ps1' -Value @'
# Apply-RecommendedPasswordPolicy (embedded via batch)
...
if errorlevel 1 (
    echo Failed to write PowerShell script to %%TEMP%%. Aborting.
    pause
    goto menu
)

echo Running the policy apply script (elevated context assumed)...
powershell -NoProfile -ExecutionPolicy Bypass -File "%TEMP%\toolbox_apply_policy.ps1"
...
net accounts
echo.
echo Exporting current policy to %%windir%%\Temp\secpol_verify.cfg ...
secedit /export /cfg "%windir%\Temp\secpol_verify.cfg" 2>nul
if exist "%windir%\Temp\secpol_verify.cfg" (
    powershell -NoProfile -Command "Select-String -Path '%windir%\Temp\secpol_verify.cfg' -Pattern 'MinimumPasswordLength|PasswordComplexity|PasswordHistorySize|ClearTextPassword|MaximumPasswordAge|MinimumPasswordAge|LockoutBadCount|ResetLockoutCount|LockoutDuration' -SimpleMatch | ForEach-Object { Write-Host $_.Line.Trim() }"
) else (
    echo Failed to export policy for verification. Ensure elevated privileges.
)
